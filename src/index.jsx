...
const App = () => {
  ...
  var [pageContent] = useState(async () => {
    return await getPageContent(pageId);
  });

  // Search for content that sits between the <dione ...> tags. Keep the tags
  ...
  // Iterate through dioneCommands and extract the second parameter after the tag name. E.g. <dione flowchart> -> flowchart
  // Then create an OpenAI prompt using the extracted data
  dioneCommands.forEach(async (command) => {
    const commandName = command.match(/<dione.*?>/g)[0].split(" ")[1].split(">")[0]
    const commandType = assignType(commandName)
    const commandData = command.match(/<dione.*?>(.*?)<\/dione>/)[1]
  ...
  const updatedContent = pageContetn.replace(command, snippet + command.replace(/<dione.*?>/g, ""))
